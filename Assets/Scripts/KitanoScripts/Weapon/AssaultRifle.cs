...
public class AssaultRifle : WeaponBase
{
    ...
    void PlayFireSEForAllClients()
    {
        ...
        {
            Debug.Log("PlayFireSE called on local player: " + Runner.LocalPlayer);
            ...
            Debug.Log("Requesting PlayFireSE on " + Runner.LocalPlayer);
    ...
    {
        Debug.Log("RPC_RequestPlayFireSE called by " + rpcinfo.Source + " on " + Runner.LocalPlayer);
        if (AudioManager.Instance == null || string.IsNullOrEmpty(fireClipKey)) { return; } // AudioManagerが存在し、クリップキーが設定されている場合
    ...
    {
        Debug.Log("RPC_ApplyPlayFireSE called by " + sourcePlayer + " on " + Runner.LocalPlayer);
